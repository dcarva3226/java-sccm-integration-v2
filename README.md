Needed a place to start storing sample java code for future re-use. This project represents a connector for SCCM.

## License
Proprietary / Custom License

Copyright (c) 2025 Danny Carvajal

All rights reserved.

Permission is granted to view the code in this repository for personal,
non-commercial purposes only.

You may NOT:
- Copy, modify, publish, distribute, or use the images in any product or service
- Remove copyright notices or watermarks
- Create derivative works based on these images
